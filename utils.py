...
def calculate_grouped_sample_weights(df: DataFrame, n_column='No_de_viajes_muestreados') -> DataFrame:
    ...
    Additional logic:
    - Also calculates original weights based directly on n_column (weight_samples_original)
    - Calculates adjust_weighted = weight_samples_original * Ajuste_por_ML
    ...
    Returns:
        DataFrame: Enhanced DataFrame with the following new columns:
            ...
            - weight_samples_original: Original weights based on n_column
            - adjust_weighted: Original weights * ML adjustment
    ...
    Process:
        ...
        5. Calculate original weights: weight_samples_original = n_column / sum(n_column)
        6. Apply ML adjustments to both weight types
    """
    
    # Define window for grouping operations by Plant and Standard
    window_partition = Window.partitionBy("Planta", "norma")
    
    # ORIGINAL WEIGHTS CALCULATION (based on n_column directly)
    # Calculate total of original samples by group
    df = df.withColumn(
        "sum_original_samples",
        _sum(col(n_column)).over(window_partition)
    )
    
    # Calculate original weights (simple proportion of n_column)
    df = df.withColumn(
        "weight_samples_original",
        when(col("sum_original_samples") == 0, lit(0))
        .otherwise(col(n_column) / col("sum_original_samples"))
    )
    
    # Calculate original weighted adjustment
    df = df.withColumn(
        "adjust_weighted",
        col("weight_samples_original") * col("Ajuste_por_ML")
    )
    ...
    df = df.drop("sum_original_samples", "sum_samples_matched", "sum_samples_unmatched", "sum_all_new_samples")
...
def compute_agg_adjusted_weights(df_week_0_5, df_adjustments, week_analysis_list, n_column='No_de_viajes_muestreados'):
    """
    Computes the sum of new_adjust_weighted and sum_adjust_weighted by Planta and norma for multiple weeks.
    ...
    -------
    pyspark.sql.DataFrame
        Aggregated DataFrame with columns: week_analysis, Planta, norma, sum_new_adjust_weighted, sum_adjust_weighted
    """
    dfs = []
    results = []
    for week_analysis_value in week_analysis_list:
        ...
        agg_df = result_df.groupBy("Planta", "norma").agg(
            ...
            _sum("adjust_weighted").alias("sum_adjust_weighted")
        )
    ...
    final_df = final_df.select("week_analysis", "Planta", "norma", "sum_new_adjust_weighted", "sum_adjust_weighted")
